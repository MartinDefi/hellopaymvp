 ...
 * @LastEditTime: 2024-04-29 18:15:01
...
const interRegularFont = fetch(
  ...
  const jersey15Font = fetch(
    new URL("/public/Jersey15-Regular.ttf", import.meta.url)
  ).then((res) => res.arrayBuffer());
  
export const app = new Frog()

app.frame('/', as (c) => {
    const [interRegularFontData, interBoldFontData, firaScriptFontData, jersey15FontData] =
    await Promise.all([interRegularFont, interBoldFont, firaScriptFont, jersey15Font]);
    const { buttonValue, inputText, status } = c
    const fruit = inputText || buttonValue
    return c.res({
        image: (
            <div tw="flex flex-col w-full h-full bg-red-500">
      <div tw="flex justify-between items-center p-10 h-1/4">
        <div style={{ fontFamily: "'Jersey15', monospace", fontSize: 60, fontWeight: 700, color: '#fff' }}>Crypto Red Package</div>
        <div tw="flex">
          ...
          <svg version="1.0" xmlns="http://www.w3.org/2000/svg"
           width="400.000000pt" height="80.000000pt" viewBox="0 0 1627.000000 321.000000"
      ...
      <div tw="flex flex-col w-full justify-center p-10 h-1/2 bg-yellow-50">
        <div tw="flex justify-between items-center px-10 py-2 w-full">
          <div style={{ fontFamily: "'Fira Code', monospace", fontSize: 35, fontWeight: 400, color: '#000' }}>
            Lucky Money Balance:
          </div>
          <div style={{ fontFamily: "'Fira Code', monospace", fontSize: 35, fontWeight: 400, color: '#000' }}>
            100 USDT
      ...
      <div tw="flex w-full justify-center h-1/4 items-center" style={{ fontSize: 50, fontWeight: 700, color: '#feff00' }}>Follow HelloPay’s Farcaster Bro!</div>
    </div>
        ),
